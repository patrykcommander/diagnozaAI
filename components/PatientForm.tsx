...
interface PatientFormProps {
  ...
  patientBloodExamination: BloodExamination | undefined;
}
...
function FormRow({ item }: FormRowProps) {
  return (
    <Grid item xs={6}>
      {" "}
      {/* Set xs to 12 if multiline is true, otherwise set it to 6 */}
      <div className="flex flex-row items-center gap-10 justify-start">
        <p className="w-2/5 font-semibold text-xl text-right text-wrap">
          {item.label}
        </p>
        <TextField
          key={item.label}
          className="w-3/5"
          value={item.value || ""}
          placeholder={item.value === null ? "Brak danych" : ""}
          variant="outlined"
        />
      </div>
    </Grid>
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center mb-20">
      <div className="flex flex-col w-1/2 align-middle gap-5">
        <div className="flex">
          <p className="flex-1 text-xl text-center font-semibold">
            Pliki źródłowe
          </p>
        </div>
        <div className="flex border-2 rounded-md border-black justify-between mb-10">
          <p className="flex-1 text-xl text-center border-r-2 border-black py-2">
            {merged.json_file_name}
          </p>
          <p className="flex-1 text-xl text-center py-2">
            {merged.csv_file_name || "Brak"}
          </p>
        </div>
      </div>
      <div>
        <Grid container spacing={4}>
          <Grid container item rowSpacing={2} columnSpacing={5}>
            {Object.entries(merged).map(
              ([key, value], index) =>
                !key.includes("file_name") &&
                !key.includes("histology") && (
                  <FormRow
                    key={index}
                    item={{
                      label:
                        DB_NAME_TO_LABEL[key as keyof typeof DB_NAME_TO_LABEL],
                      value: value,
                      fieldName: key,
                    }}
                    multiline={false}
          ...
          <Grid item xs={12}>
            <div className="flex flex-col items-center gap-5 justify-start pt-5">
              <p className="font-semibold text-xl text-right text-wrap">
                {DB_NAME_TO_LABEL["histology" as keyof typeof DB_NAME_TO_LABEL]}
              </p>
              <TextField
                className="w-full"
                rows={6}
