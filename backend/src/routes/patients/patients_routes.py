...
def managePatient():
    if request.method == "GET":
        ...
        nr_pacjenta = request.args.get("nr_pacjenta")
        if nr_pacjenta == None:
            return Response(response={"msg": "Did not receive Patient ID"}, status=404)

        response, status = Patient.deletePatient(nr_pacjenta=nr_pacjenta)
        return Response(response=response, status=status)

@app.route('/patients', methods=['GET'])
def getAllPatients():
    returnCount = request.args.get('all')
    fullDataFilter = request.args.get('fullData')
    page = request.args.get('page') 

    if fullDataFilter == "yes":
        fullDataFilter = True
    elif fullDataFilter == "no":
        fullDataFilter = False
    else:
        fullDataFilter = "all"

    if returnCount != None or returnCount == "true":
        length, status = Patient.getLength(fullDataFilter)
    ...
    response, status = Patient.fetchPaginatedPatients(page, elementsPerPage, fullDataFilter)
